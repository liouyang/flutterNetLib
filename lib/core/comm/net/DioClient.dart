...
class DioClient {
  ...
  Future<Result<T, ApiException>> _handleResponse<T>(
    ...
    try {
      ...
      Log.d('已解析 BaseResponse，路径：${response.requestOptions.path}，业务码：${baseResponse.code}，消息：${baseResponse.message}', tag: 'DioClient'); // 中文日志

      // <<< 核心修改：在这里检查业务码！
      if (baseResponse.code != 200) {
        Log.w('检测到业务错误，路径：${response.requestOptions.path}，业务码：${baseResponse.code}，消息：${baseResponse.message}', tag: 'DioClient'); // 中文日志
      ...
      if (baseResponse.data != null) {
        Log.d('请求成功并包含数据，路径：${response.requestOptions.path}', tag: 'DioClient'); // 中文日志
        return Success(baseResponse.data as T);
      } else {
        // 业务成功但 data 为 null 的情况。
        // 根据 T 是否可空来决定返回成功 (null) 还是失败。
        if (null is T) {
          Log.i('请求成功，但数据为空且类型可空，路径：${response.requestOptions.path}', tag: 'DioClient'); // 中文日志
          return Success(null as T); // 安全地返回 null
        } else {
          Log.w('请求成功，但非空数据为空，路径：${response.requestOptions.path}', tag: 'DioClient'); // 中文日志
          // 如果 T 是非空类型，但 data 为 null，视为业务逻辑上的不完整或错误
          return Failure(ApiException(
            message: baseResponse.message.isNotEmpty ? baseResponse.message : '操作成功但未返回数据，且数据类型为非空。',
            code: baseResponse.code, // 仍然使用业务成功码 200，但提示数据缺失
          ));
        }
      }
    } on TypeError catch (e, st) {
      Log.e('响应解析时发生类型错误，路径：${response.requestOptions.path}', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      // JSON 解析失败或数据模型转换失败
      throw DioException(
        requestOptions: response.requestOptions,
      ...
      Log.e('处理服务器响应时发生未知错误，路径：${response.requestOptions.path}', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      // 其他未知异常
      throw DioException(
        requestOptions: response.requestOptions,
  ...
  Future<Result<T, ApiException>> get<T>(
    ...
    try {
      ...
      Log.e('捕获到 GET 请求的 DioException，路径：$path，消息：${e.message}', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      // 所有的 DioException 都会在这里被捕获。
      // 它们都已经被 ErrorInterceptor 转换成了 ApiException。
      return Failure(e.error as ApiException);
    } catch (e, st) {
      Log.e('捕获到 GET 请求的通用错误，路径：$path，错误：$e', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
  ...
  Future<Result<T, ApiException>> post<T>(
    ...
    try {
      ...
      Log.e('捕获到 POST 请求的 DioException，路径：$path，消息：${e.message}', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      return Failure(e.error as ApiException);
    } catch (e, st) {
      Log.e('捕获到 POST 请求的通用错误，路径：$path，错误：$e', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      return Failure(ApiException(message: e.toString()));
    }
  }
  ...
  Future<Result<T, ApiException>> put<T>(
    ...
    try {
      ...
      Log.e('捕获到 PUT 请求的 DioException，路径：$path，消息：${e.message}', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      return Failure(e.error as ApiException);
    } catch (e, st) {
      Log.e('捕获到 PUT 请求的通用错误，路径：$path，错误：$e', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      return Failure(ApiException(message: e.toString()));
    }
  }
  ...
  Future<Result<T, ApiException>> delete<T>(
    ...
    try {
      ...
      Log.e('捕获到 DELETE 请求的 DioException，路径：$path，消息：${e.message}', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      return Failure(e.error as ApiException);
    } catch (e, st) {
      Log.e('捕获到 DELETE 请求的通用错误，路径：$path，错误：$e', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      return Failure(ApiException(message: e.toString()));
    }
  }
  ...
  Future<Result<T, ApiException>> upload<T>(
    ...
    try {
      ...
      Log.e('捕获到 UPLOAD 请求的 DioException，路径：$path，消息：${e.message}', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      return Failure(e.error as ApiException);
    } catch (e, st) {
      Log.e('捕获到 UPLOAD 请求的通用错误，路径：$path，错误：$e', tag: 'DioClient', error: e, stackTrace: st); // 中文日志
      return Failure(ApiException(message: e.toString()));
    }
  }
}